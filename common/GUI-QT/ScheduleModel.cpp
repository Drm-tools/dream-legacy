...
CTxSchedule::CTxSchedule(): StationsTable(), iSecondsPreview(0),
...
{
	const CStationsItem& item = StationsTable[index.row()];
	switch(role)
	{
	...
	case Qt::DisplayRole:
		...
		{
			case 0:
				return QVariant(item.strName.c_str());
			break;
			case 1:
				return QVariant(QString().sprintf("%04d-%04d",item.GetStartTimeNum(),item.GetStopTimeNum()));
				break;
			case 2:
				return QVariant(item.iFreq);
				break;
			case 3:
				return QVariant(item.strTarget.c_str());
				break;
			case 4:
				...
				if (item.rPower == (_REAL) 0.0)
					return QVariant("?");
				else
					return QVariant(item.rPower);
				break;
			case 5:
				return QVariant(QString(item.strCountry.c_str()));
				break;
			case 6:
				return QVariant(QString(item.strSite.c_str()));
				break;
			case 7:
				return QVariant(QString(item.strLanguage.c_str()));
				break;
			case 8:
				return QVariant(QString(item.strDaysShow.c_str()));
				break;
		}
	}
	return QVariant();
...
CTxSchedule::load(const string& path)
{
	...
	for(size_t i=0; i<StationsTable.size(); i++)
	{
		QString strTarget = QString(StationsTable[i].strTarget.c_str());
		QString strCountry = QString(StationsTable[i].strCountry.c_str());
		QString strLanguage = QString(StationsTable[i].strLanguage.c_str());
...
CTxSchedule::StationState CTxSchedule::CheckState(const int iPos) const
{
	/* Get system time */
	time_t ltime;
	time(&ltime);
...
bool CTxSchedule::IsActive(const int iPos, const time_t ltime) const
{
