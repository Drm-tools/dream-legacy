 ...
 *  The class also generates the AF packet header and CRC.
 *  It would be better to separate the AF into its own class since TAG packets can be carried by
 *  IPIO as well as DCP. See also TagPacketDecoder.cpp which has the same issue.
 *  
...
CTagPacketGeneratorWithProfiles::CTagPacketGeneratorWithProfiles(const char cProf)
: cProfile(cProf)
{
}

void CTagPacketGeneratorWithProfiles::SetProfile(const char cProf)
{
	cProfile = cProf;
}
